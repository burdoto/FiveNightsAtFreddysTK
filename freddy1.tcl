...
proc init { doReload } {
	...
	set chars [list freddy bonnie chica foxy golden]
	set charNames [list "Freddy" "Bonnie" "Chica" "Foxy" "Golden Freddy"]
	
	set posList(freddy) [list stage dining_room restrooms kitchen east_hall east_corner none office]
	set posList(bonnie) [list stage dining_room backstage west_hall supply west_corner left_door office]
	set posList(chica) [list stage dining_room restrooms kitchen east_hall east_corner right_door office]
	set posList(foxy) [list cove_back cove_peek cove_run west_hall office]
	set posList(golden) [list none office]
	...
	set goldenFreddy false
	
	set camList [list stage dining_room cove west_hall west_corner supply east_hall east_corner backstage kitchen restrooms]
	set camIDs [list 1A 1B 1C 2A 2B 3 4A 4B 5 6 7]
	set activeCam 0
	set camActive false
	
	set useImage() ""
	set useImage(office.dark) "img/office/office.dark-state1.png"
	set useImage(cam.west_hall) "img/cameras/west_hall/light_on.png"
	 
	set dead false
	set doorBlockDeath false
	...
	set blocked(left) false
	set blocked(right) false
	
	set poster(west_corner) "lets_party"
	
	set powerLeft 100
	set powerUsage 1
	
	if {$doReload == "doReload"} {
		.new configure -bg #999999
		reload
	} elseif {$doReload == "noReload"} {
		update
	}
}
...
proc trigger { char } {
	...
	if {$char != "golden" && $char != "foxy"} {
		if {$pos($char) < [expr [llength $posList($char)] - 2]} {
			set thisPos $pos($char)
			set nextPos [expr $pos($char) + 1]
		} elseif {$char == "freddy" && $pos($char) == [expr [llength $posList($char)] - 2]} {
			set thisPos $pos($char)
			set nextPos [expr $pos($char) + 1]
			set dead true
